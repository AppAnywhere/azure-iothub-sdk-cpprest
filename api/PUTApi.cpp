...
PUTApi::PUTApi( std::shared_ptr<ApiClient> apiClient )
...
boost::replace_all(path, U("{") U("name") U("}"), ApiClient::parameterToString(name));
    ...
    {
        throw ApiException(400, U("PUTApi->iotHubResourceCreateEventHubConsumerGroup does not produce any supported media type"));
    }
    ...
    {
        throw ApiException(415, U("PUTApi->iotHubResourceCreateEventHubConsumerGroup does not consume any supported media type"));
    }
    ...
    .then([=](web::http::http_response response)
    {
        ...
        if (response.status_code() >= 400)
        {
            throw ApiException(response.status_code()
                , U("error calling iotHubResourceCreateEventHubConsumerGroup: ") + response.reason_phrase()
        ...
        if(response.headers().has(U("Content-Type")))
        {
            ...
            if( contentType.find(responseHttpContentType) == std::string::npos )
            {
                throw ApiException(500
                    , U("error calling iotHubResourceCreateEventHubConsumerGroup: unexpected response type: ") + contentType
                    , std::make_shared<std::stringstream>(response.extract_utf8string(true).get()));
            }
        }
...
boost::replace_all(path, U("{") U("resourceName") U("}"), ApiClient::parameterToString(resourceName));
    ...
    {
        throw ApiException(400, U("PUTApi->iotHubResourceCreateOrUpdate does not produce any supported media type"));
    }
    ...
    if ( consumeHttpContentTypes.size() == 0 || consumeHttpContentTypes.find(U("application/json")) != consumeHttpContentTypes.end() )
    {
        ...
        json = ModelBase::toJson(iotHubDescription);
    ...
    else if( consumeHttpContentTypes.find(U("multipart/form-data")) != consumeHttpContentTypes.end() )
    {
        ...
        std::shared_ptr<MultipartFormData> multipart(new MultipartFormData);
                if(iotHubDescription.get())
        {
            iotHubDescription->toMultipart(multipart, U("iotHubDescription"));
        }
    ...
    {
        throw ApiException(415, U("PUTApi->iotHubResourceCreateOrUpdate does not consume any supported media type"));
    }
    ...
    .then([=](web::http::http_response response)
    {
        ...
        {
            throw ApiException(response.status_code()
                , U("error calling iotHubResourceCreateOrUpdate: ") + response.reason_phrase()
                , std::make_shared<std::stringstream>(response.extract_utf8string(true).get()));
        }
        ...
        if(response.headers().has(U("Content-Type")))
        {
            ...
            {
                throw ApiException(500
                    , U("error calling iotHubResourceCreateOrUpdate: unexpected response type: ") + contentType
                    , std::make_shared<std::stringstream>(response.extract_utf8string(true).get()));
            }
        }
